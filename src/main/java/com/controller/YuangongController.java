    ...
    */
    ...
    public R save(@RequestBody YuangongEntity yuangong, HttpServletRequest request){
        ...
        if(yuangongEntity==null){
            yuangong.setInsertTime(new Date());
            ...
            return R.error(511,"账户或者普通员工手机号或者普通员工身份证号已经被使用");
        }
    }
...
//                            yuangongEntity.setYuangongName(data.get(0));                    //普通员工姓名 要改的
//                            yuangongEntity.setYuangongPhone(data.get(0));                    //普通员工手机号 要改的
//                            yuangongEntity.setYuangongIdNumber(data.get(0));                    //普通员工身份证号 要改的
...
//                            yuangongEntity.setJiguan(data.get(0));                    //籍贯 要改的
//                            yuangongEntity.setYuangongZhuzhi(data.get(0));                    //家庭住址 要改的
//                            yuangongEntity.setRuzhiTime(new Date(data.get(0)));          //入职时间 要改的
//                            yuangongEntity.setInsertTime(date);//时间
...
//                            yuangongEntity.setCreateTime(date);//时间
                        ...
                        if(yuangongEntities_yuangongPhone.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [普通员工手机号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
                         //普通员工身份证号
                        List<YuangongEntity> yuangongEntities_yuangongIdNumber = yuangongService.selectList(new EntityWrapper<YuangongEntity>().in("yuangong_id_number", seachFields.get("yuangongIdNumber")).eq("yuangong_delete", 1));
                        if(yuangongEntities_yuangongIdNumber.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [普通员工身份证号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
    ...
    */
    ...
    public R login(String username, String password, String captcha, HttpServletRequest request) {
        ...
        String token = tokenService.generateToken(yuangong.getId(),username, "yuangong", "普通员工");
        R r = R.ok();
        r.put("token", token);
        r.put("role","普通员工");
...
//    	ValidatorUtils.validateEntity(user);
        ...
        YuangongEntity yuangongEntity = yuangongService.selectOne(queryWrapper);
        if(yuangongEntity != null)
            return R.error("账户或者普通员工手机号或者普通员工身份证号已经被使用");
        yuangong.setInsertTime(new Date());
        yuangong.setYuangongDelete(1);
        yuangong.setCreateTime(new Date());
        yuangongService.insert(yuangong);
